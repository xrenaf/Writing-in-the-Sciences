...
6. Does each body paragraph carry enough content to pass the density check —
   at least five of {claim, specific number, named case, corroborating case,
   mechanism, inline comparison, scoped caveat, citation}? See
   `references/sections/paragraph-density-mmlongbench.md` (MMLongBench
   NeurIPS 2025 exemplars) for Pattern A–C and target length ranges. An
   Introduction paragraph under 60 words is almost always a fragment

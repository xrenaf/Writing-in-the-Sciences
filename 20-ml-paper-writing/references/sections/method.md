...
Method paragraphs are *dense by default*. Target 150–250 words per module,
with every design-choice justification inline ("we use X because Y") so a
reviewer who never opens the appendix still understands the *why*. If the
reasoning for a choice is only visible in the appendix, the method paragraph
is under-written.

See `references/sections/paragraph-density-mmlongbench.md` for Pattern B
and the §3.1 VRAG exemplar from MMLongBench (NeurIPS 2025).

...
## Paragraph-Level Craft, Claim-Evidence Map, and Self-Review

The macro-level workflow above gets you to a draft. This section is what turns a draft into a paper that survives skeptical reviewers. It is mandatory before any section is considered "done."

### A. One Paragraph, One Message (Paragraph Clarity Check)

Every paragraph must satisfy all four:

1. **Single explicit message.** State it in the first sentence (the topic sentence). If you cannot summarize the paragraph in one clause, split it.
2. **Self-contained nouns.** Define each new term before reusing it; do not rely on the reader holding hidden context from three paragraphs ago.
3. **Sentence-to-sentence relation.** Each sentence connects to the previous one via a clear relation: cause, contrast, consequence, refinement, or example. If you cannot name the relation, the flow is broken.
4. **Old-before-new order.** Familiar information first, then the new claim — anchored in the stress position (sentence end).

When a paragraph "feels off," run the check above before rewriting prose.

### B. Reverse Outlining (run after every section draft)

After drafting any section, do this in writing — not in your head:

1. Write the section thesis in one sentence.
2. Under it, write the topic sentence of every paragraph.
3. Under each topic sentence, write the evidence/explanation it relies on.
4. Verify two mappings: each topic sentence supports the thesis, and each evidence point supports its topic sentence.
5. Any paragraph that cannot be cleanly mapped is either rewritten, moved, or deleted.

If flow is still weak: insert temporary section subheaders and explicit transitions ("In contrast,…", "Consequently,…") during revision; remove the unnecessary ones before finalizing.

Source distillation: `references/sections/does-my-writing-flow-source.md`.

### C. Paragraph Roles (use as scaffolding when drafting prose)

Tag each paragraph in your draft with one of these roles. A section that uses the same role twice in a row is usually redundant:

`opening` · `challenge` · `method` · `advantage` · `evidence` · `limitation` · `transition`

This makes structural problems visible (e.g., three `evidence` paragraphs in a row with no `advantage` paragraph means the reader has no idea why the evidence matters).

### D. Claim–Evidence Map (HARD CONSTRAINT for Abstract & Introduction)

Every major claim in the Abstract and Introduction must be tracked in a `Claim → Evidence → Status` table. Use this exact format:

```
Claim:    <one-sentence claim as it appears in the paper>
Evidence: <which table / figure / experiment supports it>
Status:   supported | needs evidence | weakened | removed
```

Rules:
- Any claim with status `needs evidence` must be either downgraded ("we observe", "suggests") or cut before submission.
- If the same claim appears in Abstract and Intro, it must use the same wording. Drift here is the #1 reviewer complaint.
- This map is part of your deliverable to the scientist (see Output Contract below), not a private note.

### E. Five-Dimension Self-Review (mandatory before declaring a section done)

At the end of each section draft AND before submission, answer all five out loud and revise based on unresolved items:

1. **Contribution** — Is the single contribution clear in this section? Does it match the one-sentence contribution from Step 1?
2. **Writing clarity** — Does every paragraph pass the clarity check (§A)? Does the reverse outline (§B) hold?
3. **Experimental strength** — Are claims supported by evidence stronger than the strongest baseline a skeptical reviewer would demand?
4. **Evaluation completeness** — Are obvious ablations, baselines, and failure-mode analyses present? What would Reviewer 2 ask for?
5. **Method design soundness** — Is the design motivated, not just described? Does each design choice have a stated reason?

Then perform **adversarial reading**: re-read the section as a hostile reviewer looking for a reason to reject. Resolve every high-risk question before moving on. Detailed checklist: `references/sections/paper-review.md`.

### F. Output Contract (what to return when asked to draft or revise)

When the user asks you to rewrite, draft, or revise a section, your response must contain:

1. **Mini-outline** — 3–7 bullets covering the section structure (use paragraph roles from §C).
2. **Revised paragraphs** — each labeled with its paragraph role.
3. **Self-review checklist** — the five dimensions from §E, with concrete answers (not "looks good").
4. **Claim–evidence map** — the table from §D for every major claim introduced or modified.
5. **Open questions / flagged uncertainties** — anything the scientist needs to confirm.

If a section is being polished rather than drafted, items 1 and 2 collapse into a focused diff, but items 3–5 are still required.

### H. Cut the Clutter (Stanford "Writing in the Sciences", Unit 1)

A sentence-level polish pass distilled from Kristin Sainani's Stanford course. Run this **after** §A (paragraph clarity) and §B (reverse outlining), as the last pass before declaring a section done. It is the cheapest 5–15% length reduction available at zero information cost — invaluable for fitting page limits.

**Five principles** (in priority order):

1. **Cut unnecessary words and phrases.** Learn to part with your words.
2. **Use the active voice.** "We trained X" beats "X was trained."
3. **Use strong verbs.** Concrete verbs carry meaning; `be / make / do / have` bury it.
4. **Do not turn verbs into nouns.** `analyze` > `perform an analysis`; `decide` > `make a decision`. Nominalization is the single worst habit in ML methods sections.
5. **Do not bury the main verb.** Keep subject and main verb close together; defer subordinate clauses to the end of the sentence.

**Clutter checklist** (search-and-destroy on every Abstract / Intro pass):

| Category | Cut these |
|---|---|
| Dead phrases | "as is well known", "as has been shown", "it should be noted that", "it is important to mention that" |
| Empty modifiers | "important", "interesting", "basic", "methodologic" |
| Long-for-short | "utilize" → use; "in order to" → to; "due to the fact that" → because |
| Acronyms used <3 times | spell out and delete the acronym |
| Redundant pairs | "studies and examples", "challenges and difficulties", "successful solutions" |
| Crutch adverbs | "very", "really", "quite", "basically", "actually", "essentially" |

**Three small tricks:**

- **Eliminate negatives.** "did not remember" → "forgot"; "does not have" → "lacks". Negatives force readers to do mental arithmetic.
- **Eliminate "there is / there are".** They waste the verb slot on `to be`. *"There are many ways we can arrange X"* → *"We can arrange X in many ways."*
- **Drop needless `that` and prepositions.** *"They agreed that it was true"* → *"They agreed it was true."*

**Full reference with worked examples and a calibration practice set:** `references/sections/effective-writing-stanford-unit01.md`.

#### H.2 Verbs (Stanford Unit 2)

A focused pass on verb-level prose. Run after H.1 (clutter).

- **Active voice by default.** Convert passives by asking "*WHO does what to whom?*". Use passive only in Methods when *what was done* matters more than *who did it*.
- **Use "we" / "I".** Avoiding personal pronouns does not make science more objective; it just makes prose flabby. "We propose / We find / We show" is correct at every top venue.
- **Strong verbs.** Pick verbs that already contain the adverb: "approximately reports" → "estimates"; "thinks about the future" → "projects".
- **Do not bury the main verb.** Keep subject and main verb close together; defer long modifiers to the end.
- **Nominalization sweep** (especially in Methods) — search-and-replace these:
  ...
  | provide a methodologic emphasis on | emphasize |
  | offer confirmation of | confirm |

- **Grammar tripwires reviewers notice**: `data` is plural; `affect` (verb) vs `effect` (noun); `compared with` (differences between similar things — almost always what ML papers want) vs `compared to`; restrictive `that` (no comma) vs non-restrictive `which` (commas); avoid singular `they` by pluralizing.

**Full reference:** `references/sections/effective-writing-stanford-unit02.md`.

#### H.3 Sentence and Paragraph (Stanford Unit 3)

This is the most directly applicable Stanford unit for paragraph-level craft. It operationalizes §A–C above.

**Punctuation power, low → high:** comma → colon → dash → parentheses → semicolon → period. Use:
- **Semicolons** to join two closely related independent clauses, or to separate list items that already contain commas.
- **Colons** to introduce a list, explanation, or amplification — or to join two independent clauses where the second extends the first.
- **Dashes** for *emphasis* or strong asides. They are the one legitimate way to "bury the verb": the reader recovers it just after the dash. Don't overuse them.
- **Parentheses** for genuinely skippable asides only.

**Parallelism (mandatory for ML papers):**
- Pairs joined by `and / or / but` and all lists must use parallel grammatical form.
- This is doubly important for **contribution bullets** in the Introduction — every bullet should start the same way ("We propose…", "We show…", "We release…").
- Same applies to ablation tables, experiment subsection headers, and figure captions.

**Paragraph principles (= the §A–C contract, restated):**
1. One paragraph = one idea. State the punch line in the first sentence.
2. Keep paragraphs short. White space helps reading.
3. First and last sentences are remembered best — engineer both.
4. Flow comes from (i) logical ordering, (ii) parallel sentence structure between consecutive sentences, and (iii) *minimal* transition words. Transition words cannot fix bad logic.

**Editing workflow when a paragraph "feels off"** (use this, not sentence-by-sentence rewrites):
1. Outline the paragraph in plain English: main idea + 2–4 supporting points.
2. Reorder the points into general → specific (or chronological / logical).
3. Strike anything that does not advance the main idea.
4. Re-write following the new outline, adding parallel structure where you can.

In the Stanford example, this workflow cuts a 212-word paragraph to 91 words at zero information loss.

**Other tips worth keeping:**
- **Repeat keywords** — switching to a synonym makes the reader think you mean a different concept. Acceptable repetition is better than confusing variation.
- **The Rule of Three** — three examples / three contributions / three baselines highlighted is the pleasing default. Enough to make a point, not so many that the reader drowns.
- **Acronyms**: only use standard ones (DNA, GPU, LLM). Don't abbreviate just because something recurs.

**Full reference:** `references/sections/effective-writing-stanford-unit03.md`.

#### H.4 Paragraph Density — target length and structural load per paragraph

Stanford Units 1–3 teach you to remove clutter and to lay out one paragraph
around one idea. A parallel failure, more common in ML drafts than clutter, is
the **thin analysis paragraph**: a topic sentence, one number, and a period.
These paragraphs look tidy but carry one move where a reviewer expects five,
and they are the single most common source of "lacks rigor" reviewer feedback
on otherwise-sound drafts.

Run this pass **after** H.1–H.3 — it assumes the prose is already clean; it is
asking whether each clean paragraph *does enough work*.

**Length targets (as starting reference points, not rigid rules):**

| Use case | Target length | Target substantive sentences |
|---|---|---|
| Analysis paragraph (Experiments / Findings) | 80–160 words | 6–8 |
| Method design paragraph | 150–250 words | 8–11 |
| Related Work methodological paragraph | 180–250 words | 7–10 |
| Contribution-list bullet (not a paragraph) | 1–2 lines each | — |

Anything under 60 words in a body section should trigger a "fragment or
paragraph?" check — fold it into a neighbour or expand it.

**Density checklist.** A body paragraph should carry at least five of:
(1) a claim sentence, (2) a specific numerical datapoint with context,
(3) a named concrete case (no generic "some models"), (4) a corroborating
case that rules out a one-off, (5) a mechanism or causal reading,
(6) an inline comparison to an alternative, (7) a scoped limitation or
future-work pointer, (8) a citation when borrowing from or contrasting
prior work. Fewer than five means it is a note, not an analysis paragraph.

**Three structural patterns** (apply by use case — full verbatim exemplars
from Wang et al., *MMLongBench* (NeurIPS 2025) live in the reference):

- **Analysis (Pattern A):** `[bold claim] → [scope] → [numerical example] →
  [corroborating cases] → [mechanism + citation] → [honest caveat]`.
- **Method (Pattern B):** `[capability definition] → [task framing] →
  [example] → [dataset sources + citations] → [construction recipe] →
  [design-choice justification: "we use X because Y"] → [metric + cite] →
  [appendix pointer for low-level only]`.
- **Related Work (Pattern C):** organise *by direction of argument*, not by
  paper: `[topic sentence] → [origin + cite] → [mechanism] → [extensions] →
  [critique] → [field response] → [contrast with our work]`.

**Anti-patterns to watch for:** the *orphan finding* (claim + number, nothing
else); the *generic "some models"* (name them, with scores); the *buried
mechanism* (one-word parenthetical where a full sentence is needed); the
*dangling caveat* (paragraph ends on the finding with no honest scoping,
forcing reviewers to supply the scoping themselves).

**Do not inflate with hedges, transitions, or rephrasings.** Density means
more *content*, not more words. A 160-word paragraph with seven substantive
moves beats a 220-word paragraph with three moves and a lot of "importantly",
"notably", and "it should be noted that".

**Full reference with verbatim MMLongBench paragraphs and structure
decompositions:** `references/sections/paragraph-density-mmlongbench.md`.

#### H.5 Experimental Analysis: Conclusion-Driven, Not Number-Driven

The most common failure mode of AI-assisted experimental analysis is
**number padding**: parroting model-by-model scores that the reader can
already see in the table. This produces paragraphs that read like
spreadsheet narration rather than scientific analysis. If deleting your
paragraph and just looking at the table conveys the same information,
your paragraph adds nothing.

**Core principle:** every analysis paragraph must answer **WHY**, not
just **WHAT**. State the conclusion first, then explain the mechanism
that produces the observed pattern.

Five rules:

1. **Conclusion first.** Open with the finding, not with "As shown in
   Table X." The finding is the topic sentence.
2. **Mechanism over numbers.** For each category, type, or topology,
   explain *what structural property* makes it hard or easy. One
   sentence of mechanism is worth five numbers.
3. **Numbers as anchors only.** Use a number only to establish scale
   ("a 50 pp spread") or to anchor a claim the table alone cannot
   convey. Never enumerate model-by-model scores.
4. **Relative comparisons over absolute values.** "X leads Y but
   collapses on Z" conveys more than "X achieves 55.3% on Y and 12.1%
   on Z."
5. **Name the bottleneck.** End analysis paragraphs by identifying the
   missing capability, not just the low score.

**Full reference with annotated MC-Search and MMLongBench exemplars:**
`references/sections/experiment-analysis-style.md`.

### G. Section Reference Loading Rule

`references/sections/` contains per-section guidance distilled from top-researcher writing advice. **Do not load all of them at once.** Load only the file matching the current edit target:
...
| Final review pass | `references/sections/paper-review.md` |
| Diagnosing flow | `references/sections/does-my-writing-flow-source.md` |
| Concrete examples | `references/sections/examples/index.md` |
| Sentence-level polish (cut clutter) | `references/sections/effective-writing-stanford-unit01.md` |
| Verbs / active voice / nominalization sweep | `references/sections/effective-writing-stanford-unit02.md` |
| Punctuation, parallelism, paragraph editing | `references/sections/effective-writing-stanford-unit03.md` |
| Paragraph *density* — analysis / method / related-work (MMLongBench exemplar) | `references/sections/paragraph-density-mmlongbench.md` |
| Experiment analysis *style* — conclusion-driven writing (MC-Search + MMLongBench) | `references/sections/experiment-analysis-style.md` |

These complement (do not replace) the macro-level guides in `references/writing-guide.md` and `references/checklists.md`.
...
|------------|------------|------------------------|------------------|
| **NeurIPS 2026** | 9 pages | +0 | Mandatory checklist, multi-track (Main/Position/E&D/Creative AI) |
...
cp -r templates/neurips2026/ ~/papers/my-new-paper/
...
|------------|-----------|----------------|-------|
| NeurIPS 2026 | `neurips_2026.tex` | `neurips_2026.sty` | Multi-track support, checklist.tex |
...
- **ML/AI**: ICML 2026, ICLR 2026, NeurIPS 2026, ACL/EMNLP, AAAI 2026, COLM 2025
...
**ML/AI Venues:** [NeurIPS](https://neurips.cc/Conferences/2026/PaperInformation/StyleFiles) | [ICML](https://icml.cc/Conferences/2025/AuthorInstructions) | [ICLR](https://iclr.cc/Conferences/2026/AuthorGuide) | [ACL](https://github.com/acl-org/acl-style-files)
